...
export const LuisicesStorefront: React.FC = () => {
  ...
  // Dynamic Categories from STORE_PRODUCTS
  const customCategories = useMemo(() => {
    const baseCats = [
      { id: 'todos', label: 'Todas as Coleções', icon: '✨' },
      { id: 'casamentos', label: 'Casamentos & Noivados', icon: '💍' },
      { id: 'topos_3d', label: 'Topos de Bolo 3D', icon: '✂️' },
      { id: 'maternidade', label: 'Batizados & Maternidade', icon: '🕊️' },
      { id: 'debutantes', label: '15 Anos & Debutantes', icon: '👑' },
      { id: 'corporativo', label: 'Kits Corporativos de Luxo', icon: '🏛️' },
    ];

    const knownIds = new Set(baseCats.map(c => c.id));
    const dynamic: Array<{ id: string; label: string; icon: string }> = [];

    STORE_PRODUCTS.forEach(p => {
      if (!p.category) return;
      const raw = p.category.trim();
      const catId = raw.toLowerCase().replace(/\s+/g, '_');
      if (!knownIds.has(catId) && !knownIds.has(raw)) {
        knownIds.add(catId);
        knownIds.add(raw);
        dynamic.push({
          id: raw,
          label: p.categoryLabel || (raw.charAt(0).toUpperCase() + raw.slice(1)),
          icon: '🎀',
        });
      }
    });

    return [...baseCats, ...dynamic];
  }, []);
